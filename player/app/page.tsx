    ...
    <div style={{display:"flex",flexDirection:"column",height:"100vh",overflow:"hidden",background:"var(--surface-A)"}}>
      ...
      <div style={{flex:1,minHeight:0,display:"grid",gridTemplateColumns:(chapters.length>0||talkTime.length>0)?"200px 1fr 420px":"1fr 420px",gap:12,padding:"14px 18px 18px"}}>
              ...
              <>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:2}}>
                  {chapters.map((c,i)=>{
                    ...
                    return(
                      <button key={i} onClick={()=>{seekTo(chapterToSec(c.time));videoRef.current?.play();setAutoScroll(true);}} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 10px 12px",borderRadius:10,background:isActive?"color-mix(in srgb,var(--accent) 12%,transparent)":"transparent",border:"none",cursor:"pointer",textAlign:"left",width:"100%",position:"relative"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10,minWidth:0,overflowY:"auto",scrollbarWidth:"thin",paddingRight:4}}>
          ...
          {/* SIGNALS FEED */}
          {signals.length>0&&(
            <SignalsFeed signals={signals} time={time} duration={duration} onSeek={(t)=>{
              seekTo(t);
              videoRef.current?.play();
              setAutoScroll(true); // re-enable transcript follow
